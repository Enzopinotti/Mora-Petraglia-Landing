...
export default function Prints() {
  const { products, getContent, source } = useCms()
  ...
  // En el CMS, la obra destacada debe ser explícita. Si es fallback, usa FEATURED_ARTWORK.
  const featuredProduct = products.find((p) => p.subtype === 'original_artwork' && p.featured) || products.find((p) => p.subtype === 'original_artwork')

  const showFeatured = source === 'fallback' || (source === 'cms' && !!featuredProduct)
        ...
        {printsList.length > 0 && (
          <div className="prints__grid">
            {printsList.map((product) => {
              ...
              // Lógica de botón de compra directa/consulta
              let ctaText = 'Consultar'
              let ctaHref = '#contacto'
              let ctaOnClick: any = null

              if (product.status === 'sold' || product.availability === 'sold') {
                ctaText = 'Vendido'
                ctaHref = '#'
              } else if (product.status === 'out_of_stock' || product.availability === 'out_of_stock') {
                ctaText = 'Sin stock'
                ctaHref = '#'
              } else if (product.sale_mode === 'direct' && product.payment_link) {
                ctaText = 'Comprar'
                ctaHref = product.payment_link
                ctaOnClick = (e: any) => {
                  e.stopPropagation()
                  window.open(product.payment_link, '_blank')
                }
              } else if (product.sale_mode === 'unavailable') {
                ctaText = 'No disponible'
                ctaHref = '#'
              } else {
                ctaOnClick = () => handleArtworkInquiry(product.name, sizeLabel)
              }
                ...
                <article className="print-card" key={product.id || product.name} data-reveal>
                  ...
                  <div className="print-card__copy">
                    ...
                    <div className="print-card__footer">
                      ...
                      <a href={ctaHref} onClick={ctaOnClick}>
                        {ctaText}
                      </a>
                    </div>
                  </div>
